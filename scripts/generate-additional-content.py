...
try:
    ...
    print("ERROR: google-genai パッケージが必要です → pip install google-genai", file=sys.stderr)
    sys.exit(1)
...
# generate-city-content.py と同じコンテキストデータ
CITY_CONTEXT = {
    # 北海道
...
地域の特性に合わせた、重複のないユニークで高品質な「市場トレンド解説」と「ご当地FAQ」を生成してください。

【重要なルール】
- 指定された文字数を厳守すること
- 他の都市と内容が重複しないよう、その都市固有の情報（地名・観光地・条例の雰囲気・季節需要など）を必ず含めること
- 民泊オーナー（物件を持っているが運営を任せたい人、これから始めたい人）に向けた文章にすること
"""
...
def build_prompt(city_key: str, entry: dict, context: dict) -> str:
    ...
    return f"""
以下の都市向けに、民泊運営の「市場トレンド解説（marketAnalysis）」と「ご当地FAQ（localFaqs）」を生成してください。
...
【生成するフィールド】
以下のJSON形式のみを出力してください。Markdownタグ（```json）などは含めず、純粋なJSONテキストだけを返してください。

{{
  "marketAnalysis": {{
    "title": "（{short}の民泊市場・観光トレンド。25文字以内。例：『{short}のインバウンド需要と民泊の可能性』）",
    "body": "（{short}特有の観光需要、人気の旅行者層、季節ごとの稼働率の傾向、条例や地域ルールなど、民泊運営における独自の分析コラム。必ず{short}の固有名詞（観光スポットや地名など）を2つ以上含めること。250〜350文字）"
  }},
  "localFaqs": [
    {{
      "q": "（{short}で民泊を運営する際の特有の疑問。天候・雪・虫・ゲスト層・ルールなどから1つ。40文字以内）",
      "a": "（上記に対する専門的な回答。必ず{short}の地名や事情を含める。80〜120文字）"
    }},
    {{
      "q": "（{short}特有の疑問その2。異なる切り口で。40文字以内）",
      "a": "（回答。80〜120文字）"
    }},
    {{
      "q": "（{short}特有の疑問その3。異なる切り口で。40文字以内）",
      "a": "（回答。80〜120文字）"
    }}
  ]
}}
...
def main():
    ...
    for slug, entry in cities.items():
        if slug == 'japan': continue
        if only_set and slug not in only_set:
            continue
        # marketAnalysis が既に存在する場合はスキップ
        if "marketAnalysis" in entry:
            continue
        targets.append(slug)
    ...
    for i, slug in enumerate(targets, 1):
        ...
        try:
            ...
            entry["marketAnalysis"] = generated["marketAnalysis"]
            entry["localFaqs"] = generated["localFaqs"]
            ...
            time.sleep(2) # GeminiのRate limit対策
